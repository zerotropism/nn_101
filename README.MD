...
db = a * dx
```
assuming `dx` is given or `1` by default. Because its local gradient always returns `1.0`, the `+` gate on its end gives
```python
x = a + b
da = 1.0 * dx
db = 1.0 * dx
...
# can be split in 3 simpler ones
x1 = a * b + c
x2 = x1 * d
x = x2 * x2

# allowing easier gradient computations (= backprop equations)
dx2 = 2 * x2 * dx
dd = x1 * dx2
dx1 = d * dx2
da = b * dx1
